    ...
    if stream {
        tokio::spawn(async move {
            ...
            for token in tokens {
                ...
                let delay = rand::random::<u64>() % (config.response.stream_delay_max_ms - config.response.stream_delay_min_ms + 1) + config.response.stream_delay_min_ms;
        ...
        let _ = tx.send(Ok(Event::default().data(serde_json::to_string(&response).unwrap()))).await;
        let _ = tx.send(create_done_event()).await;
    }
